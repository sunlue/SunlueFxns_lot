...
public class RealPlay {
	...
	public void handle(RealPlayCallback callback) {
		...
		if (previewSucValue == -1) {
			...
			callback.fail("预览失败【" + new Error().hikvision(errCode) + "】", 280, 160);
			return;
		}
	 ...
	 * @throws Exception
	 */
	public static void fullScreen(MouseEvent e) throws Exception {
		// 播放视频信息
		Map<String, Object> realPlay = VideoMonitor.getDeviceHikvisionRealPlay(e.getComponent().getName());
		if (realPlay == null || realPlay.size() < 1) {
			throw new Exception("无视频播放信息");
		}
		// 新建全屏预览窗口
	...
	public interface RealPlayCallback {
		 ...
		 * @param height
		 */
		void fail(String errMsg, int width, int height);
